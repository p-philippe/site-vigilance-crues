...
| **Modifier `public_html/index.html` à la main** au lieu des sources | Moyenne — c'est arrivé le 2026-08-30 | Élevé — le prochain `build.py` écrase silencieusement la modification | Toujours modifier `src/` puis rebuilder ; le contrôle 9 (`build_is_current`) détecte la désynchronisation mais **pas** une prod plus récente que les sources | Après tout `git pull`, vérifier que `python3 build.py` ne modifie pas `public_html/index.html` |
