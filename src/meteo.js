...
export function renderNappes() {
  ...
  for (const sta of NAPPES_DATA) {
    ...
    let tendance = '→', tendColor = 'var(--text3)';
    if (niveau != null && prev?.niveau_nappe_eau != null) {
      const diff = niveau - prev.niveau_nappe_eau;
      if (diff > 0.05) { tendance = '↑'; tendColor = '#2980b9'; }
      else if (diff < -0.05) { tendance = '↓'; tendColor = '#27ae60'; }
    }
    ...
    const niveauStr = niveau != null ? niveau.toFixed(2) + ' m NGF' : '—';
    const profStr = prof != null ? prof.toFixed(2) + ' m' : '—';
    const pctStr = pct != null ? `${pct}ᵉ centile` : 'chronique insuffisante';
    ...
    html += `<div class="sol-card">
      <div class="sol-card-hdr">
        ...
        <div style="font-size:18px;font-weight:700;color:${tendColor};margin-left:8px">${tendance}</div>
      </div>
      <div class="sol-gauge-wrap">
        <div class="sol-gauge-label">
          <span>Vs historique (même mois) : <strong>${pctStr}</strong></span>
          <span style="font-size:11px;font-weight:700;color:${cls.color};background:${cls.bg};padding:1px 8px;border-radius:99px">${cls.label}</span>
        </div>
        ${nappeJauge(pct, cls.color)}
        <div class="sol-kpis">
          <div class="sol-kpi"><div class="sol-kpi-l">Profondeur</div><div class="sol-kpi-v" style="font-size:13px">${profStr}</div></div>
          <div class="sol-kpi"><div class="sol-kpi-l">Niveau NGF</div><div class="sol-kpi-v" style="font-size:13px">${niveauStr}</div></div>
        </div>
        <div style="font-size:10px;color:var(--text3);margin-top:6px">Mesure du ${date || '—'}${sta.pctYears ? ` · chronique ${sta.pctYears} an${sta.pctYears > 1 ? 's' : ''}` : ''}</div>
        ${masse ? `<div style="font-size:10px;color:var(--text3);margin-top:2px">Masse d'eau : ${masse}</div>` : ''}
        <div style="margin-top:6px">
          <a href="https://ades.eaufrance.fr/Fiche/PtEau?Code=${encodeURIComponent(sta.code_bss)}" target="_blank" style="font-size:11px;color:var(--accent);text-decoration:none">↗ Fiche ADES</a>
        </div>
      </div>
    </div>`;
  }
  ...
  if (synth) {
    ...
    const badge = (label) => {
      ...
      return `<span style="background:${c[0]};color:${c[1]};padding:2px 10px;border-radius:99px;font-size:11px">${label} : ${counts[label]}</span>`;
    };
    synth.innerHTML = `<div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
      <span style="font-weight:600">${total} piézomètres</span>
    ...
    <div style="font-size:10px;color:var(--text3);margin-top:6px">Niveau de chaque piézomètre comparé à sa propre chronique historique pour le même mois (percentile). « Très haut » = capacité d'absorption du bassin quasi nulle, risque aggravé de crue et de remontée de nappe.</div>`;
  }
}
